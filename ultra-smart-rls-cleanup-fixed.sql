-- ULTRA-SMART RLS CLEANUP SCRIPT - FIXED SYNTAX
...
BEGIN
    ...
    LOOP
        ...
        ELSIF user_id_type IN ('text', 'varchar') THEN
            ...
            policy_condition := 'user_id = auth.uid()::text';
        ELSE
        ...
        sql_command := format('CREATE POLICY %I ON %I FOR ALL TO authenticated USING (%s) WITH CHECK (%s)',
            policy_name,
            table_name,
            policy_condition,
            policy_condition
        ...
        sql_command := format('CREATE POLICY %I ON %I FOR ALL TO authenticated USING (EXISTS (SELECT 1 FROM user_profiles WHERE user_profiles.user_id = auth.uid()::text AND user_profiles.is_admin = true))',
            policy_name,
            table_name
        );
        EXECUTE sql_command;
    ...
    CREATE POLICY user_profiles_select_policy ON user_profiles
        ...
        USING (
            ...
            EXISTS (
                ...
                WHERE admin_check.user_id::text = auth.uid()::text 
