-- Create stored procedure for agent safety migration
CREATE OR REPLACE FUNCTION run_agent_safety_migration()
RETURNS void
LANGUAGE plpgsql
AS $$
BEGIN
  ...
  CHECK (status IN ('pending', 'in_progress', 'needs_next_agent', 'in_review', 'ready_for_preview', 'changes_requested', 'approved', 'completed', 'cancelled'));

  -- Add agent safety columns to admin_tasks
  ...
  RAISE NOTICE 'Agent safety migration completed successfully';
END;
$$; 