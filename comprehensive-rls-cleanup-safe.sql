-- COMPREHENSIVE RLS CLEANUP - All Tables (TYPE-SAFE VERSION)
-- This script cleans up all RLS policies across the entire database
-- Handles both TEXT and UUID user_id columns safely
...
-- For tables with TEXT user_id columns
-- photo_vault (assuming TEXT user_id)
...
-- glow_checks (assuming TEXT user_id)
...
-- content_calendar (assuming TEXT user_id)
...
-- user_progress (assuming TEXT user_id)
...
-- course_progress (assuming TEXT user_id)
...
-- future_self_images (assuming TEXT user_id)
...
-- selfie_scores (assuming TEXT user_id)
...
-- purchases (assuming TEXT user_id)
...
-- vip_applications (assuming TEXT user_id)
...
-- profiles (assuming TEXT user_id)
...
SELECT 
  t.table_name,
  'RLS enabled but no policies - may need attention' as status
...
SELECT 'Type casting handled properly for UUID/TEXT compatibility!' as bonus;
