...
export default function PersonalizedDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const sandraMessage = {
        message: `${profile?.first_name || 'Beautiful'}, your confidence score jumped 12 points this week! Your "expertise without ego" content is resonating powerfully. Ready for today's breakthrough moment?`,
    ...
    <div className="min-h-screen bg-soft-white">
      ...
      <div className="bg-luxury-black text-soft-white px-6 md:px-15 py-8 flex flex-col md:flex-row justify-between items-center gap-5">
        <div className="text-center md:text-left">
          <div className="text-xl mb-2">You've posted {data.stats.postsThisMonth} times this month. That's {data.stats.monthlyGrowth} more than last month!</div>
          <div className="text-warm-gray text-sm tracking-wider uppercase">Your consistency is paying off</div>
        </div>
        <div className="text-center">
          <span className="font-serif text-4xl block">+{data.stats.newFollowers}</span>
          ...
          <div className="mb-15">
            ...
            <div className="space-y-5">
              <div className="bg-[#FAFAFA] p-8 hover:bg-luxury-black hover:text-soft-white transition-all duration-300 cursor-pointer">
                <div className="font-serif text-5xl mb-1">{data.stats.newFollowers}</div>
                <div className="text-xs tracking-wider uppercase opacity-70">New Connections</div>
              </div>
              
              <div className="bg-[#FAFAFA] p-8 hover:bg-luxury-black hover:text-soft-white transition-all duration-300 cursor-pointer">
                <div className="font-serif text-5xl mb-1">{data.stats.confidenceScore}%</div>
                <div className="text-xs tracking-wider uppercase opacity-70">Confidence Score</div>
              </div>
              
              <div className="bg-[#FAFAFA] p-8 hover:bg-luxury-black hover:text-soft-white transition-all duration-300 cursor-pointer">
                <div className="font-serif text-5xl mb-1">{data.stats.postsThisMonth}</div>
